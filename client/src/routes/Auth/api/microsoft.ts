...
export async function microsoftLogin(code: string | boolean): Promise<string | void> {
  try {
    ...
    const response = await axiosInstance.post(
      `${import.meta.env.VITE_SERVER_URI}v1/auth/login/microsoft`,
      {
        code: code,
      },
      {
...
export function microsoftInitLoginFlow(): void {
