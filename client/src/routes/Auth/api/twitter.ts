...
export async function twitterLogin(code: string | boolean): Promise<string | void> {
  try {
    ...
    const response = await axiosInstance.post(
      `${import.meta.env.VITE_SERVER_URI}v1/auth/login/twitter`,
      {
        code: code,
      },
      {
...
export function twitterInitLoginFlow(): void {
